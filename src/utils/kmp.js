...
export const filterData = (data, query, fields = [], ignoreCase = true) => {
  const { includeTerms, excludeTerms, exactPhrases, orConditions } = parseQuery(query)
  ...
  const checkField = (itemValue) => {
    ...
    // 检查包含项
    const includeMatch = includeTerms.every((term) => kmpSearch(itemValue, term, ignoreCase) !== -1)
