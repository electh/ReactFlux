...
- タッチジェスチャー、長押しコンテキストメニュー、折りたたみ式ナビゲーションに対応したレスポンシブレイアウト
- ダークモードおよびカスタムテーマカラー対応
- カスタマイズ可能な読書体験:
  - カバー画像と概要表示を設定できるカラム・リスト・カードレイアウト
  - フォントファミリーおよびフォントサイズの設定
  - 記事の横幅調整
  - タイトル配置のカスタマイズ
  - 拡大やスライドショーに対応した画像ビューアー
  - 画像プレビューと音声・動画再生に対応した記事添付ファイル
  - 脚注表示の拡張
  - ソースコードのシンタックスハイライト
  - 推定読了時間の表示
  - 閲覧方向と元記事リンクの開き方を設定可能
- 記事・フィード管理:
  - フレーズ一致、`OR` 検索、除外検索に対応した Miniflux 全文検索
  - 既読ステータスや公開日による記事の絞り込み
  - フィードの一括操作
  - アカウントごとに組み込みビュー、カテゴリー、フィードをホームページに設定可能
  - 元記事の本文取得（全文取得）
  - ハッシュ、タイトル、URL による重複記事の除外
  - スクロール時の自動既読化
- 高度な機能:
  - カスタマイズ可能なキーボードショートカット
  - 設定バックアップのインポートとエクスポート
